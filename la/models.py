...
class ListManager(models.Manager):
    ...
    def managed_by(self, user):
        qs = super(ListManager, self).filter(manager=user)
        return qs
    ...
    def member_of(self, user):
        qs = super(ListManager, self).filter(members=user)
        return qs

    def leaders(self, user):
        qs = super(ListManager, self).filter(leaders=user)
        return qs
...
class ReferenceItem(models.Model):
    created = models.ForeignKey(settings.AUTH_USER_MODEL, blank=True, null=True, related_name='created_by',
                                on_delete=models.CASCADE)
    description = models.CharField(max_length=100)
